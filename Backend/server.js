...
app.post("/users", (req,res,next) => {
    ...
    bcrypt.hash(strPassword, 10).then(hash => {
        strPassword = hash;
        pool.query('INSERT INTO tblUsers (First_Name, Last_Name, Email, UserPassword, Phone, Date_Recorded) VALUES(?, ?, ?, ?, ?, ?)',[strEmail, strFirstName, strLastName, strPassword, strPhone, strDateRecorded], function(error, results){
            if(!error){
...
//     pool.query('INSERT INTO tblEmployees (Employee_ID, Farm_Name, Employee_First_Name, Employee_Last_Name, Pay_Rate, Position, Date_Recorded) VALUES(?, ?, ?, ?, ?, ?, SYSDATE())',
...
//         if(!error){
//             if(results.length > 0){
//                 let objEmployee = new Employee(results[0].Employee_ID, results[0].Farm_Name, results[0].Employee_First_Name, results[0].Employee_Last_Name, results[0].Pay_Rate, results[0].Position, results[0].Date_Recorded);
